...
from losses.perceptual_losses.lama.utils.utils import check_and_warn_input_range
...
class VGG19PerceptualLoss(nn.Module):
    ...
    def __init__(self, normalize_inputs=True, use_cuda=False, device_id=-1):
        ...
        if use_cuda:
            if device_id >= 0:
                self.vgg.to("cuda:" + str(device_id))
            else:
                self.vgg.cuda()
    ...
    def forward(self, input, target, mask=None):
        ...
        return {
            "loss": {
                "perceptual_loss": torch.stack(losses).sum(dim=0)
            },
            "values_to_log": dict()
        }